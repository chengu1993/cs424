...
__global__ void gpu_matrixmult(FP *a,FP *b, FP *c, int n, int p, int m, int TW) {
  ...
  FP cvalue[NTB];
  for(int i=0; i<NTB; i++) cvalue[i] = 0.;
  ...
  // only the start of every adjacents tiles needs to calculate
  if(blockIdx.x % NTB == 0) {
    // handle the edge case where the last several block only need to calculate    // less than NTB blocks
    int remaining_NTB = gridDim.x - blockIdx.x;
    remaining_NTB = (remaining_NTB >= NTB) ? NTB : remaining_NTB; 
